...
`User.isActive` (`Boolean @default(true)`) được `AuthService` (`apps/backend/src/modules/auth/auth.service.ts`) check ở **3 chỗ**: `login`, `refreshTokens`, và `validateJwtPayload` (`JwtStrategy` gọi mỗi request có JWT — quan trọng nhất, vì access token đang còn hạn của tài khoản vừa bị vô hiệu hoá cũng bị từ chối ngay ở request tiếp theo, không cần token blocklist). ORG_ADMIN đổi trạng thái qua `PATCH /users/:id { isActive }` — dùng chung route CRUD user đã có (đã scope theo `organizationId`), không có route riêng `/deactivate`. Frontend (`apps/web/src/lib/api-client.ts`) tự xoá token + redirect `/login` khi gặp `401` ngoài `/auth/login` **và** `/me/password` — route sau trả `401` hợp lệ khi gõ sai mật khẩu hiện tại (không phải phiên hết hạn), `PasswordSection` tự hiện lỗi tại chỗ. Thiếu ngoại lệ này thì gõ sai mật khẩu cũ một lần là bị đăng xuất thẳng thay vì thấy lỗi trên form (bug thật, phát hiện lúc build PR #89). Thêm route mới có `401` hợp lệ-không-phải-hết-hạn thì nhớ thêm vào danh sách loại trừ này.
...
- **`/auth/login` đếm theo EMAIL đang bị dò, không theo IP** (`loginIdentityTracker` truyền qua `@Throttle({ default: { ..., getTracker } })` — decorator hỗ trợ tracker riêng cho từng route). Đây mới là thứ chặn được brute-force.
- **Vì sao không đếm theo IP**: `main.ts` set `trust proxy: 1` nên Express suy `req.ip` bằng cách trừ **đúng một** hop khỏi `X-Forwarded-For`; edge của Railway round-robin qua nhiều địa chỉ nên cùng một client ra nhiều `req.ip` khác nhau → mỗi key chỉ đếm được một phần request và không bao giờ chạm limit. Đo thật trên staging: `X-RateLimit-Remaining` chạy 4,3,2,1 rồi **nhảy lại 4** (trước khi sửa), và 4,4,3,3,2,2 (sau khi chuyển counter sang Redis — chứng minh vấn đề nằm ở **key**, không phải ở nơi lưu). Cả hai lượt đều không ra `429` sau 6–8 lần sai liên tiếp.
- `clientIpTracker` (đọc `x-envoy-external-address`, fallback `req.ip`) vẫn dùng cho giới hạn 100 req/60s toàn cục. **Header đó chưa được xác nhận là có mặt trên Railway** — giới hạn toàn cục vì thế vẫn là best-effort. **Không** đọc entry trái nhất của `X-Forwarded-For` làm fallback: client gửi header đó lên được, xoay giá trị là thoát rate limit.
- **Giới hạn còn lại**: khoá theo email không chặn kiểu rải mật khẩu qua nhiều tài khoản. Muốn kín cần một định danh client đáng tin từ edge.
- **Cách kiểm tra lại** (không cần deploy log tạm): curl `/auth/login` nhiều lần **cùng một email** rồi đọc header `X-RateLimit-Remaining` — phải giảm đều tới 0 rồi `429`, không được nhảy ngược lên.
...
- Thời lượng: `formatDuration` (0 = chưa biết → `--:--`), `formatPosition` (0 = `0:00`), `formatTotalDuration`/`formatTotalDurationExact` ("khoảng 7 giờ" / "7 giờ 15 phút", nhận thêm tham số `Translator` — xem mục i18n dưới), `formatPlaylistMeta` ("12 bài · 45 phút[ · của chuỗi]").
...
## i18n — song ngữ vi/en (`next-intl`)

- **Locale qua cookie `NEXT_LOCALE`, không qua URL segment** (`/en/...`) — cây route giữ nguyên. `apps/web/src/i18n/request.ts` đọc cookie (`cookies()` từ `next/headers`), mặc định `vi` nếu thiếu/không hợp lệ. Đổi ngôn ngữ = ghi cookie (`LanguageSection.tsx`, trang Cài đặt) + `router.refresh()` — không reload cứng, Server Component render lại với `messages` mới rồi đẩy xuống `NextIntlClientProvider` ở root layout.
- **Hằng số locale (`LOCALE_COOKIE`, `SUPPORTED_LOCALES`, `SupportedLocale`) nằm ở `apps/web/src/i18n/locales.ts`, KHÔNG phải `request.ts`.** `request.ts` import `next/headers` + `next-intl/server` (server-only) — Client Component nào import thẳng `request.ts` để lấy mấy hằng số đó sẽ kéo theo chuỗi import server-only vào bundle client (và làm Jest vỡ vì gặp cú pháp ESM chưa transform). `LanguageSection.tsx` (Client Component) import từ `locales.ts`.
- **`apps/web/next.config.ts`: `next-intl/plugin` phải lồng vào TRONG `withSentryConfig`, không thay thế** — `withSentryConfig(withNextIntl(nextConfig), {...})`. Verify bằng `curl -I` sau khi đổi, đúng quy ước đã có ở mục Sentry.
- **`messages/vi.json` + `messages/en.json`** (`apps/web/messages/`) — namespace theo khu vực màn hình (`common`, `auth`, `nav`, `dashboard`, `store`, `player`, `playlist`, `track`, `settings`), không 1 file phẳng khổng lồ cũng không 1 file/component. `common` chứa các key dùng chéo nhiều khu vực (`duration.*`, `playlistMeta.*`, `queueRemaining`, `status.*`, `roles.*`, `cancel`, `loading`) — thêm chuỗi mới dùng ở >= 2 nơi thì để đây trước, đừng lặp lại.
- **Component gọi `useTranslations('namespace')` lấy `t`, hàm thuần (không phải component/hook) nhận `t`/`Translator` làm tham số** — `lib/nav.ts` (`dashboardNavItems(role, t)`, `storeNavItems(t)`), `lib/roles.ts` (`roleLabel(role, t)`), `lib/format.ts` (`formatTotalDuration(ms, t)`, `formatPlaylistMeta(t, {...})`) đều theo pattern này vì `useTranslations()` là hook, không gọi được ngoài component. Type `Translator` export từ `lib/format.ts`, dùng chung — khai kiểu `values?: Record<string, string | number | Date>` khớp đúng chữ ký thật của next-intl (không phải `Record<string, unknown>`, nếu không `tsc` báo lỗi gán kiểu ở mọi call site).
- **`formatPlaylistMeta(t, { count, durationMs, scope? })`** gộp pattern "{count} bài · {duration}[ · của chuỗi/của quán]" lặp lại ở `PlaylistDetail`/`StorePlaylistTracks` (đều dùng bản **exact**, `formatTotalDurationExact` bên trong). **Cẩn thận:** `PlaylistCard.tsx`/`PlaylistBrowse.tsx`/`StoreHome.tsx`/`StoreDetail.tsx` hiện vẫn dùng bản **approximate** (`formatTotalDuration`, "khoảng X giờ") cho dòng "{count} bài · {duration}[ · scope]" của riêng chúng — **đừng** đổi các chỗ đó sang `formatPlaylistMeta` (đổi sẽ âm thầm làm hiển thị lệch từ "khoảng 7 giờ" sang "7 giờ 15 phút", một lần đã xảy ra và phải revert lúc code PR i18n).
- **Test Jest KHÔNG bọc `NextIntlClientProvider` thật.** `apps/web/__mocks__/next-intl.tsx` (Jest tự áp dụng cho mọi `import ... from 'next-intl'`, không cần `jest.mock()` ở từng file, vì đặt cạnh `node_modules`) đọc thẳng từ `messages/vi.json` tĩnh, nội suy `{var}` bằng regex đơn giản. Vì `vi.json` copy nguyên văn chuỗi hiện có, test cũ assert tiếng Việt **không cần sửa** khi chuyển một component sang `useTranslations()` — chỉ cần sửa khi component đó tự đổi cách gọi hàm thuần (`nav.ts`, `roles.ts`,...) yêu cầu test tự dựng `t` qua `useTranslations()` import trực tiếp (xem `nav.test.ts`, `AppShell.test.tsx`).
- **`app/global-error.tsx` KHÔNG dùng next-intl — ngoại lệ có chủ đích.** File này bắt buộc là Client Component (`error`/`reset` props) và đứng ngoài cây layout gốc, không gọi được `cookies()`. Giữ nguyên tiếng Việt hardcode + `lang="vi"`.
- **`TrackPlayButton.tsx` từng là file tiếng Anh duy nhất ngoài `LoginForm.tsx`** (`Play {title}`/`Pause {title}`) — đã dịch vào chung `track.table.play`/`.pause` cho nhất quán, không giữ ngoại lệ tiếng Anh nào ngoài dữ liệu ví dụ không cần dịch (`admin@cafe.com`, `••••••••`).
- **Backend trả message lỗi thô** (`ApiError.message`, hiện qua idiom `err instanceof Error && err.message ? err.message : t('...')`) — nhánh `err.message` **không** qua next-intl, hiện nguyên văn ngôn ngữ backend trả. Ghi nợ kỹ thuật, chưa xử lý ở PR này (cần backend trả mã lỗi thay vì câu chữ).
