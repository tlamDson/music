...
import { CreateStoreSchema, UpdateStoreSchema, JwtPayload } from '@cafe-music/shared';
import { z } from 'zod';
...
export class StoresController {
  ...
  @Roles('ORG_ADMIN')
  create(
    @Body(new ZodValidationPipe(CreateStoreSchema)) dto: z.infer<typeof CreateStoreSchema>,
  ...
  update(
    ...
    @Body(new ZodValidationPipe(UpdateStoreSchema)) dto: z.infer<typeof UpdateStoreSchema>,
    @CurrentUser() user: JwtPayload,
  ) {
  ...
  assignGroup(
    @Param('id', ParseUUIDPipe) id: string,
    @Body(new ZodValidationPipe(AssignGroupSchema)) body: { syncGroupId: string },
    @CurrentUser() user: JwtPayload,
  ) {
